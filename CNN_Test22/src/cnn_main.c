...
int main(int argc, char **argv)
{
  ...
  if(learnmode){
    ...
    // 演算モード時
    // 中間層の重みの保存
    if((fp=fopen("/mnt/weight_hidden.bin", "r+")) != NULL){
      fread(weight_hidden, sizeof(double) * HIDDEN_NUM * (POOL_OUT_NUM + 1), 1, fp);
    }
    fclose(fp);
    // 出力層の重みの保存
    if((fp=fopen("/mnt/weight_out.bin", "r+")) != NULL){
  ...
  if(learnmode){
    // 中間層の重みの保存
    if((fp=fopen("/mnt/weight_hidden.bin", "w+")) != NULL){
      fwrite(weight_hidden, sizeof(double) * HIDDEN_NUM * (POOL_OUT_NUM + 1), 1, fp);
    }
    fclose(fp);
    // 出力層の重みの保存
    if((fp=fopen("/mnt/weight_out.bin", "w+")) != NULL){
